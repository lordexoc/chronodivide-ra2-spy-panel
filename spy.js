...
(function() {
  ...
  panel.style.cssText = `
    ...
    color:#fff; font-size:11px; min-width:280px; pointer-events:none;
    text-shadow: 1px 1px 2px #000; max-height:90vh; overflow-y:auto;
  `;
  panel.innerHTML = '<div style="color:#fa0;">⏳ Waiting for game...</div>';
  document.body.appendChild(panel);

  // === AUTO-DETECT LOCAL PLAYER ===
  let cachedMyName = null;

  function findMyName(game) {
    if (cachedMyName) return cachedMyName;

    // Method 1: gservCon properties
    try {
      ...
      if (gc) {
        ...
        for (let key of Object.keys(gc)) {
          let val = gc[key];
          if (typeof val === 'string' && allNames.includes(val) && val !== '@@NEUTRAL@@') {
            cachedMyName = val;
            return cachedMyName;
          }
        }
  ...
  function updatePanel() {
    ...
    if (!game) { panel.innerHTML = '<div style="color:#fa0;">⏳ Waiting for game...</div>'; return; }
    ...
    // === RENDER ===
    let html = `<div style="color:#0f0;margin-bottom:4px;font-size:12px;">🟢 ${myName} | $${myPlayer.credits}</div>`;
    if (allies.length) {
    ...
    enemies.forEach((enemy, idx) => {
      ...
      html += `<div style="margin-top:8px;padding:6px;border-left:3px solid ${color};background:rgba(255,255,255,0.03);">`;
      html += `<div style="color:${color};font-weight:bold;font-size:12px;">🔍 ${enemy.name} | 💰 $${enemy.credits}</div>`;

      // Power
      try {
        if (enemy.powerTrait) {
          let pw = enemy.powerTrait.power || 0, dr = enemy.powerTrait.drain || 0;
          html += `<div style="color:${pw >= dr ? '#0f0' : '#f00'};">⚡ ${pw}/${dr}${pw < dr ? ' ⚠️ LOW POWER!' : ''}</div>`;
        }
      } catch(e) {}
      ...
      try {
        ...
        if (swBuildings.length) {
          html += `<div style="margin-top:3px;color:#f00;font-weight:bold;">☢️ Super Weapons:</div>`;
        ...
        if (harvesters.length) {
          html += `<div style="margin-top:3px;color:#fa0;font-weight:bold;">🚜 Harvesters (${harvesters.length}):</div>`;
        ...
        html += `<div style="margin-top:3px;color:#aaa;font-size:10px;">⚔️(${totalArmy}) ${sorted.map(([k,v])=>`${k}x${v}`).join(', ')}</div>`;
      ...
      try {
        ...
        if (myBuildings.length) {
          ...
          if (nearby.length > 0) {
            ...
            html += `<div style="padding:2px 4px;background:#600;border:1px solid #f00;border-radius:3px;margin-top:3px;">`;
  ...
  console.log('[SPY] ✓ Cheat panel active. Select a unit to auto-detect your player.');
})();
